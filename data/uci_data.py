import pandas as pd
from .augmentation import augment_features, numerize_feature
...
GENERATED_CONCEPT_PATH = "data/heart_disease/heart_disease_generated_concepts.csv"
...
class UCIDataset(torch.utils.data.Dataset):
    ...
    def numerize(self, num_num=None):
        self.dataset = numerize_feature(self.dataset, num_num=num_num)
...
class ConceptDataset(torch.utils.data.Dataset):
    def __init__(self, generated: bool = False):
        ...
        if generated:
            self.generated_data = pd.read_csv(GENERATED_CONCEPT_PATH)
            self.dataset = pd.concat([self.dataset, self.generated_data], ignore_index=True)
            print(f"Loaded generated data with {self.generated_data.shape[0]} rows and {self.generated_data.shape[1]} columns.")
